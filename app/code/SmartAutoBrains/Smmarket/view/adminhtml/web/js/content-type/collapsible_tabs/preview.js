  ...
  var Preview =
      ...
      function (_previewCollection2) {
        ...
        function Preview(contentType, config, observableUpdater) {
          ...
          _events.on("collapsible_tabs:mountAfter", function (args) {
          ...
          _events.on("collapsible_tab_item:mountAfter", function (args) {
            if (_this.element && args.contentType.parentContentType.id === _this.contentType.id) {
              _this.refreshTabs();
            }
          });

          _events.on("collapsible_tab_item:renderAfter", function (args) {
            if (_this.element && args.contentType.parentContentType.id === _this.contentType.id) {
          ...
          _events.on("collapsible_tab_item:removeAfter", function (args) {
        ...
        _proto.addTab = function addTab() {
          ...
          (0, _contentTypeFactory)(_config.getContentTypeConfig("collapsible_tab_item"), this.contentType, this.contentType.stageId).then(function (tab) {
            _events.on("collapsible_tab_item:mountAfter", function (args) {
              if (args.id === tab.id) {
                _this3.setFocusedTab(_this3.contentType.children().length - 1);

                _events.off("collapsible_tab_item:" + tab.id + ":mountAfter");
              }
            }, "collapsible_tab_item:" + tab.id + ":mountAfter");
        ...
        _proto.bindEvents = function bindEvents() {
          ...
          _events.on("collapsible_tabs:dropAfter", function (args) {
          ...
          _events.on("collapsible_tab_item:duplicateAfter", function (args) {
